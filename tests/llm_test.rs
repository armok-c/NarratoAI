...
async fn test_build_vision_message_structure() {
    ...
    let results = provider
        .analyze_images(&[img_path], "describe this image", Some(10), Some(1))
        .await;
...
async fn test_analyze_images_result_ordering() {
    ...
    let results = provider
        .analyze_images(&images, "describe", Some(1), Some(2))
        .await;
...
fn write_test_jpeg(path: &Path) -> Result<(), Box<dyn std::error::Error>> {
...
fn create_test_jpeg_path(dir: &Path) -> PathBuf {
