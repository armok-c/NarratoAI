...
const { textConfig, loading, testing } = store
...
const modelHint = computed(() => {
  const preset = getProviderPreset(textConfig.provider)
  return preset?.recommendedModel ?? null
})

async function handleTestConnection() {
  try {
    await store.testConnection('text')
