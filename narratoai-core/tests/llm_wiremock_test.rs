...
// ---------------------------------------------------------------------------
#[test]
fn test_proxy_configuration_accepted() {
...
#[test]
fn test_proxy_empty_url_rejected() {
    let result = OpenAiCompatibleProvider::new(ProviderConfig {
        api_key: "test-key".to_string(),
        model_name: "test-model".to_string(),
        base_url: "http://localhost:0/v1".to_string(),
        max_retries: 0,
        timeout_secs: 5,
        proxy_http: Some(String::new()),
        proxy_https: None,
    });
    assert!(result.is_err(), "空代理 URL 应被拒绝");
    match result {
        Err(LLMError::Configuration(msg)) => {
            assert!(msg.contains("代理配置失败"), "错误消息应提及代理: {}", msg);
        }
        Err(e) => panic!("应为 Configuration 错误, 得到: {}", e),
        Ok(_) => panic!("应返回错误"),
    }
}
