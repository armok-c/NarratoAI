...
mod tests {
    ...
    fn test_draft_content_template_parseable() {
        let value: serde_json::Value = serde_json::from_str(super::DRAFT_CONTENT_TEMPLATE)
            .expect("draft_content 模板应可解析");
        // 验证关键字段存在
        ...
        assert!(
            value["materials"]["audios"].is_array(),
            "materials.audios 应为数组"
        );
        assert!(
            value["materials"]["speeds"].is_array(),
            "materials.speeds 应为数组"
        );
        assert!(
            value["materials"]["videos"].is_array(),
            "materials.videos 应为数组"
        );
        assert!(value["tracks"].is_array(), "tracks 应为数组");
        assert_eq!(
            value["platform"]["app_id"], 3704,
            "platform.app_id 应为 3704"
        );
        assert_eq!(
            value["canvas_config"]["width"], 1920,
            "canvas_config.width 应为 1920"
        );
        assert_eq!(
            value["canvas_config"]["height"], 1080,
    ...
    fn test_draft_meta_info_template_parseable() {
        let value: serde_json::Value = serde_json::from_str(super::DRAFT_META_INFO_TEMPLATE)
            .expect("draft_meta_info 模板应可解析");
        ...
        assert_eq!(
            value["draft_is_invisible"], false,
            "draft_is_invisible 应为 false"
        );
    }
}
