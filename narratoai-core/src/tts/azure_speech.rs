...
fn azure_voice_regex() -> &'static Regex {
    ...
    RE.get_or_init(|| {
        Regex::new(r"^[a-z]{2}-[A-Z]{2}-.+Neural$").expect("Azure voice regex is valid")
    })
}
...
pub fn should_use_azure_services(voice_name: &str) -> bool {
    ...
    let check_name = name.trim_end_matches("-Female").trim_end_matches("-Male");
    ...
    async fn synthesize_once(
        ...
        if self.config.speech_key.is_empty() || self.config.speech_region.is_empty() {
            return Err(TTSError::AuthenticationFailed(
                "Azure Speech key/region 未配置".to_string(),
            ));
        }
...
mod tests {
    ...
    async fn test_azure_speech_success() {
        ...
        let result = engine
            .synthesize_once("test text", "zh-CN-YunzeNeural", &output_path, 1.0, 0.0)
            .await;
    ...
    async fn test_azure_speech_empty_text() {
        ...
        let result = engine
            .synthesize("", "zh-CN-YunzeNeural", 1.0, 0.0, &output_path)
            .await;
    ...
    async fn test_azure_speech_empty_config() {
        ...
        match result.unwrap_err() {
            TTSError::AuthenticationFailed(msg) => {
                assert!(msg.contains("未配置"), "错误消息: {}", msg)
            }
    ...
    async fn test_azure_speech_v2_suffix_stripped() {
        ...
        let result = engine
            .synthesize_once("test", "zh-CN-YunzeNeural-V2", &output_path, 1.0, 0.0)
            .await;
        assert!(result.is_ok());
    }
}
